...
const Portfolio = () => {
  ...
  const projects: Project[] = [
    {
      id: 1,
      title: t('project.analysis.title'),
      description: t('project.analysis.description'),
      image: titanicImage,
    ...
    },
    {
      ...
      image: datasetImage,
      link: 'https://www.kaggle.com/datasets/brenosilvabarros/ai4i-2020-predictive-maintenance-dataset',
    ...
    },
    {
      ...
      link: 'https://dashboard-og.vercel.app/',
      technologies: ['Power BI', 'DAX', 'SQL', 'JavaScript', 'HTML', 'CSS'],
    },
  ];
  ...
  const ProjectCard = ({ project }: { project: Project }) => (
    <div className="portfolio-card group">
      ...
      {/* Overlay — só aparece no desktop no hover */}
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex-col justify-end p-4 hidden md:flex opacity-0 group-hover:opacity-100 transition-opacity duration-300">
      ...
      <div className="md:hidden absolute bottom-0 left-0 right-0 bg-black/70 p-3">
        <h3 className="text-sm font-bold text-white mb-2 leading-tight">{project.title}</h3>
        <div className="flex flex-wrap gap-1 mb-2">
          {project.technologies.map((tech, index) => (
            <span key={index} className="px-2 py-0.5 bg-primary/80 text-white text-xs rounded">
              {tech}
            </span>
          ))}
        </div>
        <button
          onClick={() => setSelectedProject(project)}
          className="flex items-center gap-2 px-3 py-1.5 bg-white text-black rounded-lg text-xs"
        >
